...
int main (int argc, char **argv) {
	...
	/* Now compute reflected distance, find index values, add to image */
		pthread_t 		child_threads2[NUM_THREADS];
    	struct thread_args 	work_ranges2[NUM_THREADS];
    	current_start = 0;
    	range = boundry / NUM_THREADS;
    	for(j = 0; j < NUM_THREADS; j++) {
    	    work_ranges2[j].start = current_start;
    	    work_ranges2[j].end = current_start + range;
    	    current_start += range;
    	}
